...
repository as a workspace. All nine are committed with their outputs, so you can also
just read them on GitHub without running anything.
...
src/bwalloc/
  ...
  architectures.py       TCN, Transformer, DLinear/NLinear, N-BEATS/NBEATSx
  tuning.py              leak-free random search on a development prefix
...
notebooks/               Colab-first analysis notebooks, generated by notebooks/_build.py
  ...
  09_full_feature_models lifting the univariate constraint: covariates as channels
  10_modern_architectures TCN, Transformer, DLinear/NLinear, N-BEATS/NBEATSx
  11_hyperparameter_tuning search on a development prefix; what tuning was worth
  12_final_comparison    every model, default and tuned, on one fold schedule
...
then `python -m nbconvert --to notebook --execute --inplace notebooks/*.ipynb`.
