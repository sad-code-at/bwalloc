...
provisioning cost rather than forecast error, in which the cost ratio selects
the service level at inference time rather than being trained into the
model~\cite{bega2020deepcog} (Section~\ref{sec:alloc}).
...
\textbf{Cost-aware capacity forecasting.} The closest prior work to ours is
DeepCog~\cite{bega2019deepcog,bega2020deepcog}, which makes the same central argument:
an operator does not want a forecast of traffic minimising absolute error, it wants a
forecast of the \emph{capacity} to provision, and the two differ because
over-provisioning and SLA violation carry different costs. DeepCog trains a
three-dimensional convolutional network with a tunable asymmetric loss and reports
monetary cost rather than RMSE, reducing operating expenses by 273--381\% against the
best of four benchmarks at a cost ratio of $\alpha = 2$. One of those benchmarks --- a
fixed over-provisioning offset selected by exhaustive search over the evaluation data
--- is the same hindsight-tuned comparator we construct independently in
Section~\ref{sec:cost}, which we take as evidence that it is the right heuristic to
measure against.
...
different $\kappa$ requires a different model. We leave the forecaster untouched and
place the asymmetry in a calibrated quantile, so $\kappa$ becomes a parameter set at
inference through $\tau^{*} = \kappa/(1+\kappa)$ and one model serves every cost ratio.
That separation also buys a distribution-free finite-sample coverage guarantee, which a
learned loss does not provide --- a difference that matters little with DeepCog's
metropolitan-scale measurements over 470 base stations and a great deal at
$n \approx 900$. Their evaluation is at a five-minute horizon on aggregated demand;
ours is at lead times to 24 hours on single sites.

\textbf{SLA-aware prediction and slice resource allocation.} Also close to our framing
is
...
measurements, Mehri \emph{et al.}~\cite{mehri2024online} report \texttt{ARIMA(3,0,4)} at
an MSE of 513.53 against \texttt{SARIMA(1,0,1)(1,0,1,144)} at 61.78 --- an $8.3\times$
improvement obtained by setting the seasonal period to 144 steps, which is exactly 24
hours at their 10-minute sampling interval. The same mechanism, on a different
operator's data, at a different sampling rate.
...
loss~\cite{koenker1978regression}. They are calibrated by split conformal prediction,
which is
...
$-2.7$\%), which is what rules that explanation out. The hindsight-tuned fixed
margin we compare against below is not a strawman of our own construction: it is the
\texttt{MAE-post-best} baseline of~\cite{bega2020deepcog}, arrived at there by the same
reasoning.
